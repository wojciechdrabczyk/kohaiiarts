...
export default function Contact() {
    ...
    const socialLinks = [
        ...
        { name: 'Instagram', url: 'https://www.instagram.com/kohaii_arts/', icon: <BsInstagram size={32} /> },
        { name: 'Discord', url: 'invitation link to the discord server?', icon: <FaDiscord size={32} /> },
        { name: 'Newgrounds', url: 'https://kohaiiarts.newgrounds.com/art', icon: <NewgroundsIcon className="h-8 w-8" /> },
        { name: 'Patreon', url: 'https://www.patreon.com/KohaiiArts', icon: <FaPatreon size={32} /> },
    ...
    return (
        <div className="p-4 sm:px-6 md:px-10 mt-6">
            <h1 className="text-2xl sm:text-3xl font-light text-center mb-6">You can find me on</h1>
            ...
            <div className="text-center text-sm font-light text-gray-600 mb-8 px-2">
            ...
            <form onSubmit={handleSubmit} className="mx-auto max-w-md space-y-6 px-2">
                {[
                    ...
                    <div key={id} className="space-y-1">
                        <label htmlFor={id} className="flex items-baseline gap-1 text-sm font-medium text-gray-500 cursor-default">
                            <span>{label}</span>
                        ...
                        <input
                            ...
                            type={type ?? 'text'}
                            required={required}
                            className={`w-full rounded border px-4 py-3 text-sm cursor-text ${
                ...
                <div className="space-y-1">
                    <label htmlFor="message" className="flex items-baseline gap-1 text-sm font-medium text-gray-500 cursor-default">
                        <span>Message</span>
                ...
                <button
                    type="submit"
                    className="w-full rounded bg-black py-3 text-white hover:bg-gray-800 transition duration-200"
                >
                    Send
                </button>

                {status === 'success' && <p className="text-center text-green-600">Thank you! Your message has been sent.</p>}
                {status === 'error' && <p className="text-center text-red-600">Oops! Something went wrong. Please try again.</p>}
            </form>
        </div>
    );
}
